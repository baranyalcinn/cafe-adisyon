...
// Helper fonksiyon: Türkçe karakter destekli camelCase ayırıcı
...
function ProductCardComponent({
  ...
  // Performans: Sadece ürün adı değiştiğinde regex çalışır
  const formattedName = useMemo(() => formatProductName(product.name), [product.name])

  // Tek bir buton elementi döndürüyoruz, içeriği compact prop'una göre şekilleniyor
  return (
    <button
      ref={cardRef}
      onClick={handleClick}
      ...
      {compact ? (
        ...
        <>
          ...
          <div className="flex flex-col items-start gap-1.5 px-1.5 flex-1 w-full mt-1">
            ...
            <span className="text-[14px] font-black text-emerald-600 dark:text-emerald-400 tabular-nums tracking-tighter mt-auto bg-emerald-500/10 px-2 py-0.5 rounded-lg group-hover:bg-emerald-500/20 transition-colors">
              {formatCurrency(product.price)}
            </span>
          </div>
        </>
